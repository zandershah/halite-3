    ...
    for (;;) {
        ...
        // Update tasks for each ship.
        {
            ...
            for (auto& it : me->ships) {
                ...
                if (game.turn_number + closest_dropoff +
                        me->ships.size() * 0.3 >=
                    MAX_TURNS)
                ...
                // Dropoff.
                {
                    ...
                    bool ideal_dropoff =
                        ...
                        game_map->at(ship)->halite + ship->halite +
                                me->halite >=
                            DROPOFF_COST &&
                        !local_dropoffs &&
                        game.turn_number <= MAX_TURNS * 0.666;

                    if (ideal_dropoff ||
                        game_map->at(ship)->halite + ship->halite >=
                            DROPOFF_COST) {
                        me->halite += game_map->at(ship)->halite +
                                      ship->halite - DROPOFF_COST;
        ...
        while (!ships.empty()) {
            ...
            for (auto& it : ships) {
                if (game_map->calculate_distance(ship->next, it.first->next) <=
                    1)
