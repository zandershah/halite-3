...
Direction GameMap::navigate_return(shared_ptr<Ship> ship, Task task) {
    ...
    while (!pq.empty()) {
        ...
        for (size_t i = 0; i <= 5; ++i) {
            ...
            Halite delta_halite = (left_halite + constants::EXTRACT_RATIO - 1) /
                                  constants::EXTRACT_RATIO;
            delta_halite =
                min(delta_halite, constants::MAX_HALITE - ship_halite);
    ...
    if (path.back() != ship->position || path.size() == 1) {
        if (task != HARD_RETURN) mark_vis(ship->position, 1);
        return Direction::STILL;
    }

    reverse(path.begin(), path.end());
    for (size_t i = 0; i < path.size(); ++i) {
        if (task != HARD_RETURN || i + 1 != path.size()) mark_vis(path[i], i);
    }

    for (Direction d : ALL_CARDINALS) {
        if (normalize(path[0].directional_offset(d)) == path[1]) return d;
    }
    return Direction::STILL;
}
